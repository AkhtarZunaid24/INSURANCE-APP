      ...
      <div className="w-full max-w-md bg-white p-8 md:p-10 rounded-3xl border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
        ...
        <form className="space-y-6" onSubmit={handleLogin} aria-labelledby="login-heading">
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full bg-black text-white py-4 rounded-xl font-black text-lg hover:bg-zinc-800 transition-all shadow-lg disabled:opacity-50"
          >
        ...
        <div className="mt-10 flex flex-col items-center gap-4">
          <Link 
            to="/admin/login" 
            className="w-full text-center py-3 border-2 border-zinc-100 rounded-xl text-xs font-black text-zinc-400 hover:border-black hover:text-black uppercase tracking-widest transition-all"
          >
            Admin Login
          </Link>
          ...
          <p className="text-center text-zinc-500 font-bold">
            NEW RIDER?{' '}
            <Link to="/signup" className="text-black underline hover:text-yellow-600 transition-colors">
              CREATE ACCOUNT
            </Link>
          </p>
        </div>
